...
EuclideanSequencer::EuclideanSequencer()
  : currentStep(0), selectedPattern(0), isRunning(false), 
    lastStepTime(0), currentEditParam(PARAM_PLAY), currentGroup(0), 
    outputNotes(OUT_USB), outputClock(OUT_USB), outputMidiMap(true), outputOSCMap(true) {
...
void EuclideanSequencer::begin() {
  ...
  currentConfig.hits = 8; // Se quiser padrão 16, altere para 16
  currentConfig.offset = 0;
  currentConfig.note = 36;              // Kick drum (C1)
  currentConfig.velocity = 127;         // Velocidade padrão 127
...
void EuclideanSequencer::setSelectedPattern(uint8_t idx) {
  ...
  if (selectedPattern < MAX_PATTERNS) {
    ...
    if (patterns[selectedPattern].active) {
      ...
      currentConfig.steps = DEFAULT_STEPS;
      currentConfig.hits = 8;
      currentConfig.offset = 0;
      currentConfig.note = 36 + selectedPattern;  // 36, 37, 38, 39, 40, 41, 42, 43
      currentConfig.velocity = 127;
...
void EuclideanSequencer::nextEditParam() {
  if (currentGroup == 0) {
    // MAIN group: 0-8 (9 parâmetros) - ciclar dentro do grupo
    if (currentEditParam == PARAM_OFFSET) {
      // Último do grupo 0, volta ao primeiro
      currentEditParam = PARAM_PLAY;
    } else {
      currentEditParam = (EditParam)(currentEditParam + 1);
    }
  } else {
    // OUTPUT group: 100-104 (5 parâmetros) - ciclar dentro do grupo
    // Wrap entre PARAM_TEMPO .. PARAM_OSC_MAP (inclusive)
    if (currentEditParam < PARAM_TEMPO || currentEditParam >= PARAM_OSC_MAP) {
      currentEditParam = PARAM_TEMPO;
    } else {
      currentEditParam = (EditParam)(currentEditParam + 1);
    }
  }
...
void EuclideanSequencer::prevEditParam() {
  if (currentGroup == 0) {
    // MAIN group: 0-8 - ciclar dentro do grupo
    if (currentEditParam == PARAM_PLAY) {
      // Primeiro do grupo 0, vai para último
      currentEditParam = PARAM_OFFSET;
    } else {
      currentEditParam = (EditParam)(currentEditParam - 1);
    }
  } else {
    ...
    // Wrap entre PARAM_TEMPO .. PARAM_OSC_MAP (inclusive)
    if (currentEditParam <= PARAM_TEMPO || currentEditParam > PARAM_OSC_MAP) {
      currentEditParam = PARAM_OSC_MAP;
    } else {
      currentEditParam = (EditParam)(currentEditParam - 1);
    }
  }
...
void EuclideanSequencer::nextGroup() {
  ...
  // Ir para primeiro param do novo grupo
  if (currentGroup == 0) {
    currentEditParam = PARAM_PLAY;
  } else {
    currentEditParam = PARAM_TEMPO;  // Grupo 1 começa em TEMPO (101)
  }
  // nenhuma ação adicional necessária ao mudar de grupo
}
