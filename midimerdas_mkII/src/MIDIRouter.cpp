...
cs::BluetoothMIDI_Interface* MIDIRouter::ble_midi = nullptr;
	...
	for (uint8_t outIndex = 0; outIndex < 5; ++outIndex) {
		if (routingMatrix->get(inIndex, outIndex)) {
			...
			if (outIndex < 3) {
				...
				// BLE: protocolo 1
				protocolEnabled = true; //(midMapProtocol == 1);
			} else if (outIndex == 4) {
				// USB: protocolo 0
			...
			if (outIndex < 3) {
				...
				// Saídas BLE/USB: enviar mensagem completa
				if (outIndex == 3 && ble_midi) {
					ble_midi->send(ChannelMessage(header, data1, data2));
					ble_midi->sendNow();
				} else if (outIndex == 4 && usb_midi) {
